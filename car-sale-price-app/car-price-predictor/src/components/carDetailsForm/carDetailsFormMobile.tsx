...
const CarDetailsFormMobile: React.FC = () => {
  type CarDetails = {
    brand: string;
    model: string;
    trim: string;
  ...
  const Dropdown = ({ name, placeholder, options }: { name: keyof CarDetails, placeholder: string, options: string[] }) => {
    ...
    return (
      <DropdownMenu.Root>
        <DropdownMenu.Trigger ref={triggerRef} className={classNames("mt-1 block text-sm w-full p-2 text-[#FFFFFF] overflow-hidden border border-gray-300 rounded-md h-9 bg-[#ffffff9f] text-left", !carDetails[name]?"italic":"")}>{carDetails[name] || placeholder}</DropdownMenu.Trigger>
  ...
  return (
    <div className="h-full min-w-[30%] mx-auto p-4 bg-[#09213b9a] rounded-lg shadow-md" style={{ maxWidth: '400px' }}>
      <h2 className="text-lg text-white font-semibold mb-4">Car Valuation</h2>
      <form onSubmit={handleSubmit} className="space-y-4 h-full w-full">
        <div className='flex flex-col'>
          <label className="block text-sm font-medium text-white">Brand and Model</label>
          <div className='flex flex-col'>
        ...
        <div>
          <label className="block text-sm font-medium text-white">Trim</label>
          <Dropdown name="trim" placeholder="Enter trim" options={Trims} />
        </div>
        <div className='flex flex-col'>
          <label className="block text-sm font-medium text-white">Year and Mileage</label>
          <div className='flex flex-col'>
            <Dropdown name="year" placeholder="Enter year" options={years} />
            <input onChange={handleKilometersChange} placeholder="Enter mileage" className='mt-1 block text-sm w-full p-2 text-[#FFFFFF] border border-gray-300 rounded-md h-9 bg-[#ffffff9f] text-left placeholder:text-[#FFFFFF] placeholder:italic' />
          </div>
        </div>
        <div>
          <label className="block text-sm font-medium text-white">Specification</label>
          <Dropdown name="regional_specs" placeholder="Enter specification" options={spec} />
        </div>
        <div className='flex flex-col'>
          <label className="block text-sm font-medium text-white">Fuel and Transmission Type</label>
          <div className='flex flex-col'>
        ...
        <div>
          <label className="block text-sm font-medium text-white">Cylinders</label>
          <Dropdown name="no_of_cylinders" placeholder="Enter Cylinders" options={cylinders} />
        </div>
        <div className='flex flex-col'>
          <label className="block text-sm font-medium text-white">Doors and Seating Capacity</label>
          <div className='flex flex-col'>
        ...
        <div className='flex flex-col'>
          <label className="block text-sm font-medium text-white">Warranty and Seller Type</label>
          <div className='flex flex-col'>
...
export default CarDetailsFormMobile;
